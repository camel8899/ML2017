...
def main():
    ...
    ratings_df = pd.read_csv(os.path.join(base_path, 'data/train.csv'))
    ...
    users_df = pd.read_csv(os.path.join(base_path, 'data/users.csv'), sep='::',engine = 'python')
    users_age = (users_df['Age'] - np.mean(users_df['Age'])) / np.std(users_df['Age'])
    movies_df = pd.read_csv(os.path.join(base_path, 'data/movies.csv'), sep='::',engine = 'python')
    ...
    model = build_model(max_user_id, max_movie_id, args.dim, users_info, movies_info)
    model.compile(loss='mse', optimizer='adam')
    model.summary()
    checkpoint = ModelCheckpoint('best_nn.h5', monitor='val_loss', verbose=0,
